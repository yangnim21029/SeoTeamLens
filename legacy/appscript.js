...
function runReportGenerator(config) {
  ...
  const sourceData = sourceSheet
    .getRange(2, 1, sourceSheet.getLastRow() - 1, maxCol)
    .getValues();
  ...
  if (whereConditions.length === 0) {
    Logger.log(
      `在來源工作表 "${sourceSheetName}" 中沒有找到任何有效的查詢條件。`,
    );
    return;
  }
  ...
  try {
    ...
    if (responseCode === 200) {
      ...
      Logger.log(
        `請求失敗，狀態碼: ${responseCode}, 回應: ${response.getContentText()}`,
      );
    }
  } catch (e) {
  ...
  for (const result of allResults) {
    ...
    reportItem.positions.set(
      formattedDate,
      parseFloat(result.avg_position).toFixed(2),
    );
  }

  // --- 步驟 5: 整理並寫入最終報告 ---
  if (reportData.size > 0) {
    const sortedData = Array.from(reportData.values()).sort((a, b) =>
    ...
    Logger.log(
      `報告 "${destinationSheetName}" 已成功生成，共處理 ${dataToWrite.length} 個關鍵字組合。`,
    );
    ...
    if (dataToWrite.length > 1) {
      ...
      for (let i = 1; i < dataToWrite.length; i++) {
        if (dataToWrite[i][0] !== dataToWrite[i - 1][0]) {
          ...
          if (numRowsToMerge > 1) {
            destSheet
              .getRange(mergeStartRow, 1, numRowsToMerge, 1)
              .mergeVertically();
          }
          mergeStartRow = i + 2;
        }
      ...
      if (numRowsInLastBlock > 1) {
        destSheet
          .getRange(mergeStartRow, 1, numRowsInLastBlock, 1)
          .mergeVertically();
      }
    ...
    Logger.log(
      `沒有從 API 獲取到任何數據，報告 "${destinationSheetName}" 為空。`,
    );
  }
}
