...
function DashboardOverview({ data, windowDays }) {
  ...
  const keywordSearchRows = data.keywordSearchRows || [];
  const tagSearchSummary = data.tagSearchSummary || [];
  const keywordMissingRows = data.keywordMissingRows || [];
  const primaryDomain = data.primaryDomain || null;
  const topPagesUp = data.pageMoversUp || [];
  const topPagesDown = data.pageMoversDown || [];
  const topQueriesUp = data.queryMoversUp || [];
  const topQueriesDown = data.queryMoversDown || [];
  ...
  return (
    <div className="space-y-4">
      ...
      {(keywordSearchRows.length > 0 || tagSearchSummary.length > 0 || keywordMissingRows.length > 0) && (
        <div className="w-full space-y-3">
          ...
          <div className={`flex flex-wrap items-start gap-4 ${showComparison ? "" : "xl:flex-nowrap"}`}>
            <div
              className={`flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm ${
                showComparison ? "min-w-[320px]" : "min-w-[200px] xl:max-w-[540px]"
              }`}
            >
