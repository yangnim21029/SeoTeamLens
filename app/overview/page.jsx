...
const weekdayLabel = (fullDate) => {
  ...
  if (!match) return "";
  const [, y, m, d] = match;
  const weekday = new Date(Date.UTC(Number.parseInt(y, 10), Number.parseInt(m, 10) - 1, Number.parseInt(d, 10))).getUTCDay();
...
export default function OverviewPage() {
  ...
  if (pageTrafficError) {
    return <div className="text-sm text-rose-500">載入失敗：{pageTrafficError}</div>;
  }
  ...
  return <DashboardOverview data={overviewData} windowDays={windowDays} keywordMetricsReady={keywordMetricsReady} />;
}

function DashboardOverview({ data, windowDays, keywordMetricsReady }) {
  const [showComparison, setShowComparison] = useState(false);
  const windowLabel = Number.isFinite(windowDays) ? `${windowDays.toLocaleString()} 天` : "近期";
  const avgRankDisplay = data.avgRankCurrent != null
    ? data.avgRankCurrent > MAX_VISIBLE_RANK
      ? `>${MAX_VISIBLE_RANK}`
      : data.avgRankCurrent.toFixed(1)
    : "—";
  const avgDelta = formatDelta(data.avgRankDelta, { precision: 1, helper: data.comparisonHelper });
  const competitionRate = data.decliningUnique && data.keywordsWithRankCurrent
    ? data.decliningUnique / data.keywordsWithRankCurrent
    : null;
  const competitionCategory = competitionRate == null
    ? "—"
    : competitionRate > 0.1
      ? "高"
      : competitionRate > 0.05
        ? "中"
        : "低";
  const competitionDescriptor = competitionCategory === "高"
    ? "高度"
    : competitionCategory === "中"
      ...
      : competitionCategory === "低"
        ? "低度"
        : competitionCategory;
  const competitionTone = competitionRate == null
    ? "text-slate-500"
    : competitionRate > 0.1
      ? "text-rose-500"
      : competitionRate > 0.05
        ? "text-amber-500"
        : "text-emerald-500";
  const top10PercentValue = data.top10Share != null ? data.top10Share * 100 : null;
  const top10PercentPrevValue = data.top10SharePrevious != null ? data.top10SharePrevious * 100 : null;
  const top10Percent = top10PercentValue != null ? top10PercentValue.toFixed(1) : null;
  const top10PercentPrev = top10PercentPrevValue != null ? top10PercentPrevValue.toFixed(1) : null;
  const top10CountPrev = data.top10PreviousCount ?? null;
  const top10CountDelta = top10CountPrev != null ? data.currentTop10 - top10CountPrev : null;
  const top10Delta = top10CountDelta != null ? formatDelta(top10CountDelta, { precision: 0, suffix: " 個" }) : null;
  const impressionsDisplay = data.impressionsCurrent != null ? Math.round(data.impressionsCurrent).toLocaleString() : "—";
  const impressionsDelta = formatDelta(data.impressionsDelta, { precision: 0, helper: data.comparisonHelper, suffix: " 次" });
  const clicksDisplay = data.clicksCurrent != null ? Math.round(data.clicksCurrent).toLocaleString() : "—";
  const clicksDelta = formatDelta(data.clicksDelta, { precision: 0, helper: data.comparisonHelper, suffix: " 次" });
  const ctrDisplay = data.ctrCurrent != null ? `${(data.ctrCurrent * 100).toFixed(1)}%` : "—";
  const ctrDelta = formatDelta(
    data.ctrDelta != null ? data.ctrDelta * 100 : null,
    { precision: 1, helper: data.comparisonHelper, suffix: "pp" },
  );
  const rankedKeywordsDisplay = data.keywordsWithRankCurrent != null ? data.keywordsWithRankCurrent.toLocaleString() : "—";
  const rankedKeywordsDelta = formatDelta(data.keywordsWithRankDelta, { precision: 0, helper: data.comparisonHelper, suffix: " 個" });
  ...
  const keywordSearchRows = keywordMetricsLoaded ? data.keywordSearchRows || [] : [];
  const tagSearchSummary = keywordMetricsLoaded ? data.tagSearchSummary || [] : [];
  const keywordMissingRows = keywordMetricsLoaded ? data.keywordMissingRows || [] : [];
  ...
  const formatDeltaText = (value, unitLabel = "") => {
    if (value == null || Number.isNaN(value) || value === 0) return `Δ ±0${unitLabel}`;
    const sign = value > 0 ? "+" : "";
    ...
    () =>
      trafficTimeline.map((point) => {
        ...
        const impressions = parseMetric(
          point.impressions ?? point.totalImpressions ?? point.sumImpressions ?? point.impressionsCurrent ?? null,
        );
        const clicks = parseMetric(point.clicks ?? point.totalClicks ?? point.sumClicks ?? point.clicksCurrent ?? null);

        let ctrPercent = null;
        const ctrRaw = parseMetric(point.ctr ?? point.clickThroughRate ?? point.ctrCurrent ?? null);
        if (ctrRaw != null) {
  ...
  const impressionsDomain = useMemo(() => {
    const values = chartData.map((row) => row.impressions).filter((val) => val != null && Number.isFinite(val));
    if (!values.length) return [0, 1];
    const min = Math.min(...values);
    const max = Math.max(...values);
    const span = max - min;
    const padding = span === 0 ? Math.max(Math.abs(max) * 0.1, 1) : Math.abs(span) * 0.15;
  ...
  const clicksDomain = useMemo(() => {
    const values = chartData.map((row) => row.clicks).filter((val) => val != null && Number.isFinite(val));
    if (!values.length) return [0, 1];
    const min = Math.min(...values);
    const max = Math.max(...values);
    const span = max - min;
    ...
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <SummaryCard
          ...
          value={competitionRate != null ? `${(competitionRate * 100).toFixed(1)}%` : "—"}
        ...
        <SummaryCard
          ...
          value={data.currentTop10 != null ? data.currentTop10.toLocaleString() : "—"}
          delta={showComparison && top10Delta?.label ? { ...top10Delta, helper: undefined } : null}
          description={(
            <span>
              <span className={`font-mono ${deltaClassName(top10CountDelta ?? 0)}`}>
                {top10CountDelta != null ? `${top10CountDelta > 0 ? '+' : ''}${top10CountDelta.toLocaleString()} 個` : '—'}
              </span>
              ...
              <AreaChart data={chartData} margin={{ top: 24, right: 24, bottom: 12, left: 12 }}>
                ...
                <Tooltip
                  content={({ active, payload }) => {
                    ...
                    const impressions = payload.find((item) => item.dataKey === "impressions");
                    const clicks = payload.find((item) => item.dataKey === "clicks");
                    ...
                    return (
                      <div className="rounded-md bg-white px-3 py-2 text-xs text-slate-900 shadow-lg border border-slate-200">
                        ...
                        {impressions && <div>曝光：{Number(impressions.value).toLocaleString()} 次</div>}
                        {clicks && <div>點擊：{Number(clicks.value).toLocaleString()} 次</div>}
      ...
      {(topPagesUp.length || topPagesDown.length || topQueriesUp.length || topQueriesDown.length) > 0 && (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <MoverList title="Top 5 上升頁面" items={topPagesUp} emptyLabel="尚無資料" />
          <MoverList title="Top 5 下滑頁面" items={topPagesDown} emptyLabel="尚無資料" />
          <MoverList title="Top 5 上升關鍵字" items={topQueriesUp} emptyLabel="尚無資料" />
          <MoverList title="Top 5 下滑關鍵字" items={topQueriesDown} emptyLabel="尚無資料" />
        </div>
      )}
      ...
      <div className="w-full space-y-3">
        ...
        >
          <div
            className={`flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm ${showComparison ? "min-w-[320px]" : "min-w-[200px] xl:max-w-[540px]"
              }`}
          >
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-3">
              <div>
                <span className="text-sm font-semibold text-slate-800">關鍵字搜尋量</span>
            ...
            <div>
              <table className="min-w-full table-auto border-separate border-spacing-0 text-left text-sm">
                <thead className="bg-slate-50 text-slate-500">
                  <tr>
                    <th className="px-4 py-2 font-medium" style={{ minWidth: `${keywordColumnMinWidth}px` }}>
                      關鍵字
                    </th>
                    <th className="w-20 px-4 py-2 font-medium text-right">搜尋量</th>
                    {showComparison && <th className="w-20 px-4 py-2 font-medium text-right">上一期</th>}
                    {showComparison && <th className="w-20 px-4 py-2 font-medium text-right">Δ</th>}
                ...
                <tbody>
                  {!keywordMetricsLoaded && (
                    <tr>
                      <td className="px-4 py-3 text-slate-400" colSpan={keywordTableColumnCount}>
                        指標載入中…
                      </td>
                    </tr>
                  )}
                  ...
                  {keywordMetricsLoaded &&
                    keywordSearchRows.map((row, idx) => (
                      <tr key={`${row.query}-${idx}`} className="border-t border-slate-100 text-slate-700">
                        <td className="px-4 py-2 align-top">
                          <div className="min-w-0 whitespace-normal break-words">
                            <div className="font-medium text-slate-800">{row.query}</div>
                          </div>
                        </td>
                        <td className="px-4 py-2 text-right align-top font-mono text-[13px] text-slate-800">
                          <div>{Math.round(row.impressions).toLocaleString()}</div>
                        </td>
                        {showComparison && (
                          <td className="px-4 py-2 text-right align-top text-[11px] text-slate-500">
                            {Math.round(row.impressionsPrev || 0).toLocaleString()}
                          </td>
                        )}
                        {showComparison && (
                          <td className="px-4 py-2 text-right align-top text-[11px] font-mono">
                            <span className={deltaClassName(row.impressionsDelta)}>{formatDeltaText(row.impressionsDelta, " 次")}</span>
                          </td>
                        )}
                        <td className="px-4 py-2 align-top text-slate-500">{row.tag || "—"}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
          ...
          <div className="flex flex-1 min-w-[260px] flex-col gap-4 lg:flex-row lg:flex-none lg:basis-[640px]">
            <div className="flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:min-w-[360px]">
              <div className="border-b border-slate-200 px-5 py-3">
                <span className="text-sm font-semibold text-slate-800">Tag 搜尋量排名</span>
              </div>
                ...
                {!keywordMetricsLoaded ? (
                  ...
                  <ol className="space-y-2 text-sm text-slate-700">
                    {tagSearchSummary.map((row, idx) => {
                      ...
                      const share = totalTagImpressions ? impressions / totalTagImpressions : 0;
                      const percentLabel = totalTagImpressions ? `${(share * 100).toFixed(1)}%` : "";
                      return (
                        <li key={row.tag} className="relative overflow-hidden rounded-lg border border-slate-100 bg-slate-50/40 p-3 last:mb-0">
                          {totalTagImpressions && (
                            <div
                              className="absolute inset-y-2 left-2 rounded-md bg-sky-100/70"
                              style={{ width: `${Math.min(100, Math.max(8, Math.round(share * 100)))}%` }}
                            />
                          )}
                          <div className="relative grid grid-cols-[auto_1fr_auto_auto_auto] items-center gap-3">
                            ...
                            <div className="text-xs text-slate-500 truncate" title={row.region || row.site || ""}>
                              {row.region || row.site || ""}
                            </div>
                            <span className="text-right">
                              <span className="block font-mono text-[12px] text-slate-700">{Math.round(impressions).toLocaleString()}</span>
                              {percentLabel && <span className="block text-[11px] text-slate-400">{percentLabel}</span>}
                            </span>
                            {showComparison && (
                            ...
                            {showComparison && (
                              <span className={`text-right text-[11px] font-mono ${deltaClassName(row.impressionsDelta)}`}>
            ...
            <div className="flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:w-72">
              <div className="border-b border-slate-200 px-5 py-3">
                <span className="text-sm font-semibold text-slate-800">區間無數據字詞</span>
              </div>
              <div className="px-4 py-3">
                {!keywordMetricsLoaded ? (
                  <div className="text-sm text-slate-400">指標載入中…</div>
                  ...
                  <ol className="space-y-2 text-sm text-slate-700">
                    {keywordMissingRows.map((row, idx) => (
                      <li key={`${row.query}-${idx}`} className="flex items-baseline justify-between gap-3 border-b border-slate-100 pb-2 last:border-0 last:pb-0">
                        <span className="flex flex-col">
                          <span className="font-medium text-slate-800">{row.query}</span>
                          <span className="text-xs text-slate-400">{row.tag || "未分類"}</span>
                        </span>
                        <span className="font-mono text-[12px] text-slate-600">{row.volume ? row.volume.toLocaleString() : "—"}</span>
                      </li>
                    ))}
                  </ol>
                )}
