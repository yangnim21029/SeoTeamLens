...
const Sparkline = memo(function Sparkline({
  ...
  const hasSecondary = Array.isArray(secondaryData) && secondaryData.length === len;
  const now = new Date();
  ...
  const series = useMemo(() => {
    ...
    return data.map((raw, i) => {
      ...
      const secondary = hasSecondary ? normalizeValue(secondaryData[i], effectiveSecVariant) : null;
      const iso = `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, "0")}-${String(d.getUTCDate()).padStart(2, "0")}`;
      const weekday = d.getUTCDay();
      return { primary, secondary, date: label, isoDate: iso, weekday };
    });
  }, [data, anchorUTC, len, variant, hasSecondary, secondaryData, effectiveSecVariant]);
  ...
  const weekendBands = useMemo(() => {
    ...
    for (let i = 0; i < series.length; i += 1) {
      ...
      if (point.weekday === 6) {
        ...
        bands.push({ key: `${point.isoDate}-weekend`, start: point.date, end: nextWeekday.date });
      } else if (point.weekday === 0) {
        const prev = series[i - 1];
        if (!prev || prev.weekday !== 6) {
          const nextPoint = series[i + 1] ?? point;
          bands.push({ key: `${point.isoDate}-sunday`, start: point.date, end: nextPoint.date });
        }
      }
    }
    return bands;
  ...
  const numericPrimary = useMemo(
    () => series.map((row) => row.primary).filter((v) => v != null && Number.isFinite(v)),
    [series],
  );
  const numericSecondary = useMemo(
    () => series.map((row) => row.secondary).filter((v) => v != null && Number.isFinite(v)),
    [series],
  );

  const lineColor = color || VARIANT_DEFAULT_COLORS[variant] || VARIANT_DEFAULT_COLORS.metric;
  const lineSecondaryColor = secondaryColor || VARIANT_DEFAULT_COLORS[effectiveSecVariant] || "#94a3b8";
  const gradientIdPrimary = useMemo(() => `spark-${variant}-primary-${Math.random().toString(36).slice(2)}`, [variant]);
  const gradientIdSecondary = useMemo(() => `spark-${variant}-secondary-${Math.random().toString(36).slice(2)}`, [variant]);
  ...
  if (variant === "rank") {
    ...
    const allValues = numericSecondary.length ? numericPrimary.concat(numericSecondary) : numericPrimary;
    const min = Math.min(...allValues);
    const max = Math.max(...allValues);
    const span = max - min;
    const padding = span === 0 ? Math.max(Math.abs(max) * 0.1, 1) : Math.abs(span) * 0.1;
          ...
          <LineChart data={series} margin={{ top: 4, right: 4, bottom: 4, left: 4 }}>
            ...
            <Tooltip
              content={({ active, payload }) => {
                ...
                const secondaryPoint = payload.find((item) => item.dataKey === "secondary");
                return (
            ...
            />
            <Line type="monotone" dataKey="primary" dot={false} strokeWidth={2} isAnimationActive={false} stroke={lineColor} />
            {hasSecondary && (
              <Line
                type="monotone"
                dataKey="secondary"
                ...
                strokeDasharray={variant === "percent" || effectiveSecVariant === "percent" ? "4 4" : undefined}
              />
            )}
          </LineChart>
        </ResponsiveContainer>
  ...
  return (
    <div className={className}>
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={series} margin={{ top: 2, right: 4, bottom: 4, left: 4 }}>
          <defs>
            ...
            {hasSecondary && (
              <linearGradient id={gradientIdSecondary} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={lineSecondaryColor} stopOpacity={0.45} />
                <stop offset="95%" stopColor={lineSecondaryColor} stopOpacity={0.05} />
              </linearGradient>
            )}
          </defs>
